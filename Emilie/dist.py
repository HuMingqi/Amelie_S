...
def dists(example_image_feature_vector, compare_feature_vectors_file_path):
    ...
    for image_feature_vector in list(feature_vectors_dic.values()):
        dist = dist_between_feature_vector(example_image_feature_vector, image_feature_vector)
        dists.append(dist)

    return list(zip(list(feature_vectors_dic.keys()), dists))
...
def calculate_and_save_dists(example_image_path):
    ...
    save_data.save_data('D:\\dists\\dists_of_'+examplae_image_name+'.txt', list(feature_vectors_dic.keys()), dists)
